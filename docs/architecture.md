...
**Phase 19** (자세히: [reports/19-white-dog-and-walking-kid.md](./reports/19-white-dog-and-walking-kid.md)):
- 재생 마커를 흰색 강아지("흰둥이")로 리컬러링. 추적 중 "현재 위치" 마커를 오리지널 디자인의 걷는 아이 아이콘으로 전환(`setCurrentLocationMarker`의 `tracking` 옵션) — 저작권 있는 캐릭터(짱구)의 실제 디자인은 복제하지 않고 같은 목적의 자체 디자인으로 대체.
